...
1. Create a non-game WebView app with the immutable app name `k-beauty-agent` and display name `뷰티인덱스`.
...
- Product-related videos are fetched only after the user accepts the service terms/privacy notice and taps the YouTube section. Discovery uses a Korean review query and prioritizes Korean audio metadata, Hangul titles, and Korean channel names before English fallbacks. These public YouTube results do not affect product ranking or evidence confidence, and the feature falls back to a Korean-first product search page when the server key or quota is unavailable. Configure the key only in Render after enabling YouTube Data API v3 and restricting the credential to that API.
