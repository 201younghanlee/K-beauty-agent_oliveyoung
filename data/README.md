...
`catalog_generated.csv` is a transformed snapshot of the official Open Beauty Facts JSONL export. `catalog_manifest.json` records the source URL, source timestamp, processing counts, quality thresholds, output hash, attribution, and applicable licenses.

The manifest also reports the distribution of `source_updated_at` ages. A newly downloaded daily dump does not imply that every community-contributed product record was recently edited.
...
Open Beauty Facts `countries` tags describe markets where a product was reported, not a reliable country of manufacture. Generated records therefore keep `country=Unknown` instead of presenting a sale market as product origin.
